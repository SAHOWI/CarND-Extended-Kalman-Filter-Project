...
// V3 = CalculateRMSE( V1, V2)
VectorXd Tools::CalculateRMSE(const vector<VectorXd> &estimations,
                              const vector<VectorXd> &ground_truth) {
    // define and initialioze the Vectore rmse
    VectorXd rmse(4);
    rmse << 0,0,0,0;
    ...
    // if all good, go ahead and calculate the rmse
...
// END Tools::CalculateRMSE()


// M1 = CalculateJacobia(V1)
MatrixXd Tools::CalculateJacobian(const VectorXd& x_state) {
  // define the Jacobioan Matrix Hj
  MatrixXd Hj(3,4);
  ...
  if ( x_state.size() != 4 ) {
	...
	// finally compute the Jacobian matrix
	Hj << (px/c2), (py/c2), 0, 0,
		  -(py/c1), (px/c1), 0, 0,
